...
public class TestClient {
	...
	static void testService() throws Exception {
		...
		try {
			...
			eliminarClienteCorrecto(implService);
			
			// eliminación incorrecta
			eliminarClienteIncorrecto(implService);
	...
	/**
	 * Elimina un cliente que no existe
	 * 
	 * @param implService implementación del servicio
	 * @throws Exception error en test
	 */
	private static void eliminarClienteIncorrecto(Service implService) throws Exception {
		try {
			System.out.println("Eliminar cliente incorrecto");
			implService.eliminarCliente("12"); 
			...
			throw new RuntimeException("Error grave eliminando cliente ", ex);
		}
	...
	/**
	 * Elimina un cliente correctamente
	 * 
	 * @param implService implementación del servicio
	 * @throws Exception error en test
	 */
	private static void eliminarClienteCorrecto(Service implService) throws Exception {
		Connection con = null;
		Statement st = null;
		ResultSet rs = null;
		try {
			System.out.println("Eliminar cliente correcto");
			implService.eliminarCliente("1111111F"); 

			con = pool.getConnection();

			st = con.createStatement();
			rs = st.executeQuery("SELECT count(0) FROM CLIENTE where nif = '1111111F'");
			...
			if (cadenaEsperada.equals(resultado.toString())) {
				System.out.println("\tOK cliente eliminado correctamente");
			} else {
				System.out.println("\tERROR eliminando cliente");
			}
			rs.close();
			
			rs = st.executeQuery("SELECT count(0) FROM COMPRA where nif = '1111111F'");
			...
			if (cadenaEsperada.equals(resultado.toString())) {
				System.out.println("\tOK cliente eliminado correctamente de la tabla compras");
			} else {
				System.out.println("\tERROR eliminando cliente de la tabla compras");
			}
			rs.close();
			con.commit();
		} catch (Exception ex) {
